"""WhatsApp stage 1 — query generation entry point.

Architecture: 1A (LLM term expansion) + 1B (Python combinator). Both steps
live in `shared/query_builders.py` parameterized on `anchor`; this file just
picks the WhatsApp anchor and wires the steps together for the WhatsApp
pipeline.

See `shared.query_builders` for the algorithm and rationale.
"""
...
# Substring Google should anchor on. Unquoted to dodge Serper's exact-phrase
# block on the starter tier; the substring still pulls Google toward
# invite-bearing pages, and snippets typically surface the full URL anyway.
...
async def generate_queries(session: AsyncSession, campaign: Campaign) -> list[Query]:
    ...
    logger.info(
        "stage 1 (whatsapp) done for campaign %s: %d terms × %d platforms → %d queries",
...
async def snowball_from_verified_names(
    ...
    The campaign must be re-runnable for the new queries to actually get
    processed — caller is responsible for re-enqueuing `run_campaign`.
    """
    ...
    for ld in leads:
        ...
        if not name or len(name) > MAX_GROUP_NAME_AS_TERM_LEN:
            continue
        name = name.replace('"', "")  # embedded quotes break outer quoting
        key = name.lower()
    ...
    logger.info(
        "snowball (whatsapp): campaign %s seeded %d new queries from %d names",
